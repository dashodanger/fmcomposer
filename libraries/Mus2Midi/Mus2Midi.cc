...
static bool WriteTime(uint32_t time, std::ostringstream &midioutput)
{
    ...
    for (;;)
    {
        ...
        midioutput.write((const char *)&writeval, 1);
...
static bool WriteEndTrack(std::ostringstream &midioutput)
{
    ...
    midioutput.write((const char *)endtrack, 3);
...
static bool WritePressKey(uint8_t channel, uint8_t key, uint8_t velocity, std::ostringstream &midioutput)
{
	// Write queued time
    if (!WriteTime(queuedtime, midioutput))
        return false;
    ...
    midioutput.write((const char *)&working, 1);
...
static bool WriteReleaseKey(uint8_t channel, uint8_t key, std::ostringstream &midioutput)
{
...
static bool WritePitchWheel(uint8_t channel, int16_t wheel, std::ostringstream &midioutput)
{
	// Write queued time
    if (!WriteTime(queuedtime, midioutput))
        return false;
...
static bool WriteChangePatch(uint8_t channel, uint8_t patch, std::ostringstream &midioutput)
{
	// Write queued time
    if (!WriteTime(queuedtime, midioutput))
        return false;
...
static bool WriteChangeController_Valued(uint8_t channel, uint8_t control, uint8_t value, std::ostringstream &midioutput)
{
	// Write queued time
    if (!WriteTime(queuedtime, midioutput))
        return false;
...
static bool WriteChangeController_Valueless(uint8_t channel, uint8_t control, std::ostringstream &midioutput)
{
...
static bool ReadMusHeader(std::istream& file, musheader *header)
{
    file.read((char *)header, sizeof(musheader));
    return (!file.fail());
}
...
bool mus2mid(std::istream& musinput, std::ostringstream& midioutput)
{
    ...
    musinput.seekg(musfileheader.scorestart, std::ios_base::beg);
	...
	midioutput.clear();
	midioutput.write((const char *)midiheader, sizeof(midiheader));
    tracksize = 0;
    ...
    while (!hitscoreend)
    {
        ...
        while (!hitscoreend)
        {
            ...
            musinput.read((char *)&eventdescriptor, 1);
            ...
            switch (mus_event)
            {
                ...
                case mus_presskey:
                    ...
                    if (key & 0x80)
                    {
                        musinput.read((char *)&channelvelocities[channel], 1);
                ...
                case mus_systemevent:
                    musinput.read((char *)&controllernumber, 1);
                ...
                case mus_changecontroller:
                    ...
                    musinput.read((char *)&controllervalue, 1);
    ...
    midioutput.seekp(MIDI_TRACKLENGTH_OFS, std::ios_base::beg);
    ...
    midioutput.write((const char *)tracksizebuffer, 4);

    return !midioutput.fail();
}